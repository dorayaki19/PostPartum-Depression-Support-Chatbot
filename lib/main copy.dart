...
class ChatMessage {
  ...
  ChatMessage({required this.text, required this.isUser, this.thoughtProcess});
}
...
class ChatProvider extends ChangeNotifier {
  ...
  String initStatus = 'Waking up Mindful Guide...';
  ...
  Future<void> _initializeEnvironment() async {
    try {
      ...
      initStatus = 'Verifying local model...';
      notifyListeners();
      ...
      await FlutterGemma.installModel(
        ...
        if (p < 100) {
          ...
          initStatus = 'Installing Brain: $p%';
          notifyListeners();
        }
      }).install();

      initStatus = 'Loading model into DX12 VRAM...';
      ...
      chat = await model.createChat(
        systemInstruction:
            'You are a compassionate, gentle, and helpful mental health companion. Always respond empathetically in the language used by the user. Keep responses concise and comforting.',
      );

      isInitializing = false;
      notifyListeners();
    } catch (e) {
      initStatus = 'Initialization Error: $e';
      notifyListeners();
    }
  }
  ...
  Future<void> sendMessage(String userText) async {
    ...
    try {
      ...
      messages.add(
        ChatMessage(text: "Failed to process message: $e", isUser: false),
      );
      isGenerating = false;
      notifyListeners();
    }
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        scrollbarTheme: ScrollbarThemeData(
          thumbColor: WidgetStateProperty.all(AppColors.sage200),
...
class _LoginScreenState extends State<LoginScreen> {
  final TextEditingController _nameController = TextEditingController(text: "Amiy");
  final TextEditingController _ageController = TextEditingController(text: "32");
...
class _ScreeningScreenState extends State<ScreeningScreen> {
  final List<Map<String, String>> questions = [
    ...
    {'id': 'irritable', 'label': 'Irritable towards others'},
...
class _ChatScreenState extends State<ChatScreen> {
      ...
      children: [
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  Text(
                    'Secure, local & private AI companion',
                    style: AppStyles.sansBody.copyWith(fontSize: 14),
                  ),
                ],
              ),
        ...
        Expanded(
          child: AnimatedBuilder(
            animation: _chatProvider,
            builder: (context, child) {
              if (_chatProvider.isInitializing) {
                return Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      ...
                      const SizedBox(height: 24),
                      Text(
                        _chatProvider.initStatus,
                        style: AppStyles.sansBody.copyWith(
                          fontSize: 16,
                          fontWeight: FontWeight.w500,
                        ),
                      ),
                      if (_chatProvider.downloadProgress > 0) ...[
                        const SizedBox(height: 16),
                        SizedBox(
                          width: 250,
                          child: ClipRRect(
                            ...
                            child: LinearProgressIndicator(
                              value: _chatProvider.downloadProgress,
                              color: AppColors.sage600,
              ...
              return ListView.builder(
                ...
                padding: const EdgeInsets.all(48),
                itemCount:
                    _chatProvider.messages.length +
                    (_chatProvider.isGenerating ? 1 : 0),
                itemBuilder: (context, index) {
                  if (index == _chatProvider.messages.length) {
                    return _buildActiveGenerationBubble();
                  }
                  return _buildMessageBubble(_chatProvider.messages[index]);
                },
              );
            },
          ),
                ...
                child: Row(
                  children: [
                      ...
                      child: AnimatedBuilder(
                        ...
                        builder: (context, child) {
                          return TextField(
                            ...
                            onSubmitted: _chatProvider.isGenerating
                                ? null
                                : (_) => _handleSend(),
                    ...
                    AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        return IconButton(
                          onPressed: _chatProvider.isGenerating
                              ? null
                              : () => _handleSend(),
                          icon: _chatProvider.isGenerating
                              ? const SizedBox(
                                  width: 20,
                                  height: 20,
                          ...
                          style: IconButton.styleFrom(
                            backgroundColor: _chatProvider.isGenerating
                                ? AppColors.sage300
                                : AppColors.sage600,
                            padding: const EdgeInsets.all(16),
                          ),
  ...
  Widget _buildActiveGenerationBubble() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (_chatProvider._currentThought.isNotEmpty)
          Container(
            margin: const EdgeInsets.only(bottom: 12, left: 16, right: 64),
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              color: AppColors.sage50.withOpacity(0.5),
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: AppColors.sage100),
            ),
            child: Column(
        ...
        if (_chatProvider._currentResponse.isNotEmpty)
          Align(
            ...
            child: Container(
              ...
              child: Text(
                _chatProvider._currentResponse,
                style: AppStyles.sansBody.copyWith(
                  color: AppColors.sage800,
                  fontSize: 16,
                  height: 1.5,
